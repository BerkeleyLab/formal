...
contains
  ...
  module procedure scalar_3D_postmultiply_real
  ...
  module procedure scalar_3D_premultiply_real
    lhs_x_rhs =  rhs * lhs
  end procedure
  ...
  module procedure scalar_3D_gradient
    ...
    associate(dx => (self%x_max_ - self%x_min_)/self%cells_)
     ...
     !  call_julienne_assert((.all. (matmul(transpose(self%gradient_operator_1D_%assemble()), p) .approximates. b/dx .within. 3E-3)))
  ...
  module procedure scalar_3D_assign_divergence
     ...
     associate( &
       ...
       lhs%points_(1,1,1,1)%values_(  :   ,  :   ,  1   ) = 0E0 ! z_min boundary
       lhs%points_(1,1,1,1)%values_(  :   ,  :   ,z_last) = 0E0 ! z_max boundary
     end associate
