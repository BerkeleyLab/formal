...
contains
  ...
  module procedure c2f_constructor
    ...
    select case(order)
    case(2)
      centers_to_faces_1D%first_ = (2E0                                 )/2
      centers_to_faces_1D%upper_ = (reshape([real::], [0,3]))/2
      centers_to_faces_1D%inner_ = ([1E0,1E0]                           )/2
      centers_to_faces_1D%lower_ = (reshape([real::], [0,3]))/2
      centers_to_faces_1D%final_ = (2E0                                 )/2
    case(4)
      centers_to_faces_1D%first_ = 1E0
      centers_to_faces_1D%upper_ = reshape([-16,  70, 70, -14,   2], [1,5])/112E0
      centers_to_faces_1D%inner_ =              [ -7,  63, 63,  -7]        /112E0
      centers_to_faces_1D%lower_ = reshape([  2, -14, 70,  70, -16], [1,5])/112E0
      centers_to_faces_1D%final_ = 1E0
    case default
  ...
  module procedure f2c_constructor
    ...
    select case(order)
    case(2)
      faces_to_centers_1D%first_ = (2E0                               )/2
      faces_to_centers_1D%upper_ = reshape([real::], [0,3])/2
      faces_to_centers_1D%inner_ =                            [1E0,1E0]/2
      faces_to_centers_1D%lower_ = reshape([real::], [0,3])/2
      faces_to_centers_1D%final_ = (2E0                               )/2
    case(4)
      faces_to_centers_1D%first_ = 1E0
      faces_to_centers_1D%upper_ = reshape([35, 140, -70, 28, -5], [1,5])/128E0
      faces_to_centers_1D%inner_ =         [-8,  72,  72, -8]            /128E0
      faces_to_centers_1D%lower_ = reshape([-5, 28, -70, 140, 35], [1,5])/128E0
      faces_to_centers_1D%final_ = 1E0
    case default
